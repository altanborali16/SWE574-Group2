...
const PostsView = ({ posts }) => {
    ...
    const [comments, setComments] = useState(posts.reduce((acc, post) => {
        acc[post.id] = []; 
        return acc;
    }, {}));

    // State to hold new comment text for each post
    const [newComment, setNewComment] = useState(posts.reduce((acc, post) => {
        acc[post.id] = ""; 
        return acc;
    }, {}));
    ...
    const handleAddComment = (postId) => {
        if (newComment[postId].trim() !== "") {
            setComments(prevState => ({
                ...prevState,
                [postId]: [...prevState[postId], { text: newComment[postId], replies: [] }]
            }));
            setNewComment(prevState => ({
                ...prevState,
                [postId]: ""
        ...
        if (newReply[`${postId}-${commentIndex}`]?.trim() !== "") {
            setComments(prevState => ({
                ...
                [postId]: prevState[postId].map((comment, index) =>
                    index === commentIndex
                        ? { ...comment, replies: [...comment.replies, newReply[`${postId}-${commentIndex}`]] }
                        : comment
                )
            }));
            setNewReply(prevState => ({
    ...
    const handleSaveEdit = () => {
        setComments(prevState => ({
            ...prevState,
            [editComment.postId]: prevState[editComment.postId].map((comment, index) =>
                index === editComment.commentIndex ? { ...comment, text: editComment.text } : comment
            )
        }));
    ...
    const handleDropdownToggle = (postId, commentIndex) => {
        setDropdownVisible(prevState => ({
            postId: prevState.postId === postId && prevState.commentIndex === commentIndex ? null : postId,
            commentIndex: prevState.commentIndex === commentIndex && prevState.postId === postId ? null : commentIndex
        }));
    };
            ...
            <div className="posts-container">
                {posts.map((post) => (
                    <div key={post.id} className="post-card">
                        <h2> <span>Topic:</span> {post.title}</h2>
                        <hr></hr>
                                ...
                                {comments[post.id].length > 0 ? (
                                    comments[post.id].map((comment, index) => (
                                        <div key={index} className="comment-item">
                                            {editComment.postId === post.id && editComment.commentIndex === index ? (
                                                        ...
                                                        {replyFormVisible[`${post.id}-${index}`] && (
                                                            <div className="add-reply">
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => handleReplyChange(post.id, index, e.target.value)}
